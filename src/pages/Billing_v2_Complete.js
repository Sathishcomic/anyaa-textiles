            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                  ...
                  <span style={{ fontSize: 9, fontWeight: 700, color, background: 'rgba(0,0,0,0.02)', padding: '2px 8px', borderRadius: 4 }}>{label}</span>
                </div>
              </div>
            );
          })}
...
export default function Billing() {
  ...
  const buildInvoice = (overrideInvoiceId) => ({
    ...
    phone: custPhone,
    lineItems: rows.map(({ _uid, _productId, _variantId, _basePrice, _availableStock, ...rest }) => rest),
  ...
  return (
    <>
      <style>{`
        ...
        .bill-table .row-error { background: #fff5f5; border: 1px solid #ffcccc; }
        ...
        .row-error-msg { font-size: 10px; color: #c04070; font-weight: 700; }
        .rm-btn {
        ...
        .prod-drop-item.prod-disabled { background: #f5f5f5; }
            ...
            <table className="bill-table">
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  const hasError = errors[row._uid];
                  return (
                    <React.Fragment key={row._uid}>
                      <tr className={hasError ? 'bill-table row-error' : ''}>
